...
const createProduct = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (data.data && typeof data.data === 'string') {
            ...
            console.log("after creating parse", data)
        }

        const { name, description, price, category,categoryId, stock, } = data;
        ...
        if (req.files && Array.isArray(req.files)) {
            for (const file of req.files) {
                try {
                    ...
                    throw new AppError(500, "Failed to upload one of the images");
                }
            }
        }

        const result = await productService.createProduct({
            name, description, price: parseFloat(price), category,categoryId, stock: parseInt(stock), productImages: {
                create: images
            }, 
